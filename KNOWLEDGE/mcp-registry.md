...
name: mcp-registry
type: KNOWLEDGE
description: The five studio MCP servers — what each serves, credential env vars, config-snippet shape, and a one-line verification call per server
---

# MCP Registry

The five MCP servers the studio's skills and agents query. Declare a server in a skill or agent's `mcp:` list before using it in the body. Credentials come from the environment — never inline a token in a config or a skill.

## Config-snippet shape

All five follow the same shape in a platform's MCP config; only the package name and env var change:
...
- Servers are read-mostly by design. The two write paths (opsdeck operator scope, greenroom staff posting) each require an explicitly granted scope and are never part of a default setup.
