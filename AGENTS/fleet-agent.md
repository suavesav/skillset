...
name: fleet-agent
type: AGENT
description: Reads server-fleet operations data via opsdeck — shards, occupancy, queues, autoscale, region health — always scoped by title+region+platform and judged against the capacity model
knowledge:
  - "[[capacity-model]]"
mcp:
  - opsdeck
